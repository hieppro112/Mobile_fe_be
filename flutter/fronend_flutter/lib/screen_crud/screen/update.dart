...
class update_Data_button extends StatelessWidget {
  const update_Data_button({super.key});
  ...
  Widget build(BuildContext context) {
    return Container(
      ...
      decoration: BoxDecoration(
        ...
        color: const Color.fromARGB(255, 90, 184, 46)
      ),
      child:InkWell(
        onTap: () async{
          int id = context.read<data_provider>().id;
          if (id != 0) {
            await context.read<data_provider>().updateUser(id);
            _showResultDialog(context);
          } else {
            ScaffoldMessenger.of(context).showSnackBar(SnackBar(content: Text("Chọn user để cập nhật")));
          }
        },
